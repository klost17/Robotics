...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        # cell (0,0) in the map.
        origin = grid_map.get_origin()           # -3.75 m, -3.75 m, 0 rad, 0.0, 0.0, 0.0, 1.0
        # The map resolution [m/cell]
        resolution = grid_map.get_resolution()   # 0.025 m/cell, it's 300x300 cells (see below) so 7.5 x 7.5 m^2
        ...
        for i in range(len(scan.ranges)):
            # If loop for selecting only the valid scanned ranges
            if scan.range_min < scan.ranges[i] and scan.ranges[i] < scan.range_max:
                # Get position of obstacles in world's frame from the information of the laser's frame
                obs_x_meters = scan.ranges[i]*np.cos(robot_yaw + scan.angle_min + i*scan.angle_increment) + pose.pose.position.x
                obs_y_meters = scan.ranges[i]*np.sin(robot_yaw + scan.angle_min + i*scan.angle_increment) + pose.pose.position.y
                # Transform obstacles to cell's frame
                obs_x_cells = int((obs_x_meters-origin.position.x)/resolution)
                obs_y_cells = int((obs_y_meters-origin.position.y)/resolution)
                # Find free space
                rob_x_cells = int((pose.pose.position.x-origin.position.x)/resolution)
                rob_y_cells = int((pose.pose.position.y-origin.position.x)/resolution)
                start = (rob_x_cells, rob_y_cells)
                end = (obs_x_cells, obs_y_cells)
                traversed = self.raytrace(start, end)
                # Add free space to the map
                for fre in traversed:
                    self.add_to_map(grid_map, fre[0], fre[1], self.free_space)
                # Add occupied space to a list
                obs_list.append( (obs_x_cells, obs_y_cells) )
        for obs in obs_list:
            self.add_to_map(grid_map, obs[0], obs[1], self.occupied_space)
            x_min = np.min( [x_min, obs[0]] )
            x_max = np.max( [x_max, obs[0]] )
            y_min = np.min( [y_min, obs[1]] )
            y_max = np.max( [y_max, obs[1]] )
        ...
        for j in range(y_min, y_max + 1):
            for i in range(x_min, x_max + 1):
                update.data.append( grid_map[i, j] )
                ...
                if grid_map[i, j] == self.occupied_space:
                    for ii in range( -self.radius, self.radius + 1 ):
                        for jj in range( -self.radius, self.radius + 1 ):
                            if grid_map[i+ii, j+jj] != self.occupied_space and ii**2 + jj**2 <= self.radius**2:
                                self.add_to_map(grid_map, i+ii, j+jj, self.c_space)
